...
public:
    ...
    inline QPen mainGridPen()
    {
        return m_mainPen;
    }
    inline QPen secondaryGridPen()
    {
...
void GraphicsPlotItemPrivate::compose()
{
    ...
    // Composite by height
    qreal dataHeight =
        rect.height() - 2 * titleText->boundingRect().height() - 2 * (abscissText->boundingRect().height());
    ...
    ordinateMainNocks->setPos(-ordinateMainNocks->boundingRect().width(),
                              -5 * ordinateMainNocks->font().pointSizeF() / 4.0);

    m_sceneDataRect.setRect(rect.width() - dataWidth, 2 * titleText->boundingRect().height(), dataWidth, dataHeight);

    abscissText->setPos((dataWidth - abscissText->boundingRect().width()) / 2.0 + m_sceneDataRect.y(),
                        rect.bottom() - abscissText->boundingRect().height());
...
void GraphicsPlotItemPrivate::calculateAndSetTransForm()
{
    ...
    QTransform transform = QTransform::fromTranslate(-gridItem->rect().x() * scaleX + m_sceneDataRect.x(),
                                                     -gridItem->rect().y() * scaleY + m_sceneDataRect.y());
...
GraphicsPlotItem::GraphicsPlotItem(QGraphicsItem *parent)
    : QGraphicsItem(parent), d_ptr(new GraphicsPlotItemPrivate(this))
{
}
...
void GraphicsPlotItem::setAutoGrid(bool value)
{
    d_ptr->setAutoGrid(value);
    if (!value)
        d_ptr->gridItem->setMainGridPen(QPen(Qt::white, 0, Qt::SolidLine));
    else
        d_ptr->gridItem->setMainGridPen(QPen(Qt::black, 1, Qt::SolidLine));
}
...
Graphics2DPlotGrid::Graphics2DPlotGrid(QGraphicsItem *parent) : QGraphicsItem(parent)
{
...
void GraphicsPlotItemPrivate::setAutoGrid(bool enable)
{
    isAutoGrid = enable;
}
...
void GraphicsPlotItemPrivate::calculateAbscissGrid()
{
    ...
    if (fabs(r.width()) < std::numeric_limits<float>::min() * 5.0 ||
        fabs(r.height()) < std::numeric_limits<float>::min() * 5.0)
        return;
